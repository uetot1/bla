...
class CompressionModel(nn.Module):
    ...
    @staticmethod
    def get_gaussian_bits(values, scales, mask):
        scales = scales.clamp(0.11, 16.0)
        denominator = scales * math.sqrt(2.0)
        upper = 0.5 * (1.0 + torch.erf((values + 0.5) / denominator))
        lower = 0.5 * (1.0 + torch.erf((values - 0.5) / denominator))
        return (-torch.log2((upper - lower).clamp_min(1e-9)) * mask).sum()

    def quantize_z_and_get_bits(self, z, qp):
        z_hat = self.quantize_ste(z)
        index = torch.full((z.shape[0],), qp, dtype=torch.long, device=z.device)
        upper = self.bit_estimator_z(z + 0.5, index)
        lower = self.bit_estimator_z(z - 0.5, index)
        bits = -torch.log2((upper - lower).clamp_min(1e-9)).sum()
        return z_hat, bits

    def forward_prior_train_2x(self, y, common_params, y_spatial_prior):
        q_dec, scales, means = common_params.chunk(3, 1)
        q_dec = q_dec.clamp_min(0.5)
        y = y / q_dec
        batch, channel, height, width = y.shape
        masks = self.get_mask_2x(batch, channel, height, width, y.dtype, y.device)
        y_hat = 0
        bits = 0
        for index, mask in enumerate(masks):
            residual = (y - means) * mask
            bits = bits + self.get_gaussian_bits(residual, scales, mask)
            current = (self.quantize_ste(residual) + means) * mask
            y_hat = y_hat + current
            if index == 0:
                scales, means = y_spatial_prior(torch.cat((y_hat, common_params), dim=1)).chunk(2, 1)
        return y_hat * q_dec, bits

    def forward_prior_train_4x(self, y, common_params, y_spatial_prior_reduction,
        ...
        reduced_params = y_spatial_prior_reduction(common_params)
        y = y * q_enc
        batch, channel, height, width = y.shape
        masks = self.get_mask_4x(batch, channel, height, width, y.dtype, y.device)
        adaptors = (y_spatial_prior_adaptor_1, y_spatial_prior_adaptor_2,
        ...
        for index, mask in enumerate(masks):
            residual = (y - means) * mask
            bits = bits + self.get_gaussian_bits(residual, scales, mask)
            y_hat = y_hat + (self.quantize_ste(residual) + means) * mask
            if index < 3:
                params = torch.cat((y_hat, reduced_params), dim=1)
                scales, means = y_spatial_prior(adaptors[index](params)).chunk(2, 1)
        return y_hat * q_dec, bits
