...
class IntraEncoder(nn.Module):
    ...
    def forward(self, x, quant_step):
        ...
        if self.training or not CUSTOMIZED_CUDA_INFERENCE or not x.is_cuda:
...
class DMCI(CompressionModel):
    ...
    def forward_train(self, x, qp):
        ...
        z_hat, z_bits = self.quantize_z_and_get_bits(z, qp)
        params = self.y_prior_fusion(self.hyper_dec(z_hat))
        params = params[:, :, :y.shape[2], :y.shape[3]].contiguous()
        y_hat, y_bits = self.forward_prior_train_4x(
            y, params, self.y_spatial_prior_reduction,
            self.y_spatial_prior_adaptor_1, self.y_spatial_prior_adaptor_2,
            self.y_spatial_prior_adaptor_3, self.y_spatial_prior)
        x_hat = self.dec(y_hat, curr_q_dec).clamp(0, 1)
        bpp = (y_bits + z_bits) / (x.shape[0] * x.shape[2] * x.shape[3])
        return x_hat, bpp

    def forward_reconstruction(self, x, qp):
        curr_q_enc = self.q_scale_enc[qp:qp+1]
        curr_q_dec = self.q_scale_dec[qp:qp+1]
        y = self.enc(x, curr_q_enc)
        z_hat, _ = round_and_to_int8(self.hyper_enc(self.pad_for_y(y)))
        params = self.y_prior_fusion(self.hyper_dec(z_hat))
        params = params[:, :, :y.shape[2], :y.shape[3]].contiguous()
        *_, y_hat = self.compress_prior_4x(
            y, params, self.y_spatial_prior_reduction,
            self.y_spatial_prior_adaptor_1, self.y_spatial_prior_adaptor_2,
            self.y_spatial_prior_adaptor_3, self.y_spatial_prior)
        return self.dec(y_hat, curr_q_dec).clamp(0, 1)
