...
class MaskedConv2d(nn.Conv2d):
    ...
    def forward(self, input, padding=True):
        ...
        if not self.training:
            ...
            return self._conv_forward(input, self._w_cache, self.bias) #Hadi added bias
        else:
            self._w_cache = None
            # return self.conv2d_forward(input, self.weight*self._mask)
            # return self._conv_forward(input, self.weight*self._mask, self.bias*self._mask)  # Hadi added bias
            return self._conv_forward(input, self.weight * self._mask, self.bias)  # Hadi added bias
...
class ContextModel(nn.Module):
    ...
    def __init__(self, num_features, num_phi_features, entropy_model, kernel_size=5):
        ...
        #Hadi added:
        # self.condition_size = 2#self.entropy_model.condition_size
