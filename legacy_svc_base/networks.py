    ...
    def __init__(self, num_filters, num_features, num_hyperpriors,
        ...
        for i in range(num_layers):
            ...
            self.add_module('synthesis'+str(i), AugmentedNormalizedSynthesisTransform(
                out_channels, num_features, num_filters[i], kernel_size, use_affine=use_affine and i != num_layers-1, distribution=init_code))
...
class CANFHyperPriorCoderWithTemporalPrior(CANFHyperPriorCoder):
    def __init__(self, in_channels_tp=3, num_filters_tp=None, **kwargs):
        ...
        #Hadi added:----------------------------
        if self.use_context:
            self.conditional_bottleneck.condition_size = 2
        #---------------------------------------
...
class MyANFIC(HyperPriorCoder):
    ...
    def __init__(self, num_filters, num_features, num_hyperpriors,
        ...
        if use_context:
            self.hyper_synthesis = GoogleHyperSynthesisTransform(
                num_features*2, hyper_filters, num_hyperpriors)
            #Hadi added---------------------------------------------
            self.pred_prior = GoogleAnalysisTransform(in_channels,
                                                      num_features,
                                                      num_filters[0],
                                                      kernel_size)

            self.PA = nn.Sequential(
                nn.Conv2d(num_features * 3, 640, 1),
                nn.LeakyReLU(inplace=True),
                AttentionBlock(640),
                nn.Conv2d(640, 640, 1),
                nn.LeakyReLU(inplace=True),
                nn.Conv2d(640, num_features*2, 1)
            #--------------------------------------------------------
            )
        elif self.use_mean or "Mixture" in condition:
            self.hyper_synthesis = GoogleHyperSynthesisTransform(
    ...
    def forward(self, input, code=None, jac=None, temporal_cond=None):
        # Encode
        ori_input = input
        jac = [] if jac else None
