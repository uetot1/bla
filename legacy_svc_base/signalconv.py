...
class SignalConv2d(Conv2d):
    ...
    def forward(self, input):
        # for torch==1.7
        return self._conv_forward(input, self.weight if self.parameterizer is None else self.parameterizer(self.weight, self.kernel_size),self.bias) #Hadi added last bias
        # for torch==1.4
...
class SignalConvTranspose2d(ConvTranspose2d):
    ...
    def forward(self, input, output_size=None):
        ...
        output_padding = self._output_padding(
            input, output_size, self.stride, self.padding, self.kernel_size, 0) #Hadi added 0
