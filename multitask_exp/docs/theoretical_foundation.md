...
**Hình.** Kiến trúc đa tác vụ, mở rộng từ kiến trúc một tác vụ của bài RIVF (Fig. 2). Vùng A (backbone DCVC-RT) giữ nguyên không đổi. Vùng B mở rộng từ một nhánh (chỉ detection) thành hai nhánh song song: nhánh detection giữ nguyên kiến trúc gốc, BatchNorm của bản sao giữ ở eval mode như script huấn luyện của bài (nhãn **FIX**; xem ghi chú đính chính bên dưới), nhánh segmentation là thành phần hoàn toàn mới (nhãn **MOI**). Nhãn **FZ** (xanh) = đóng băng, **TR** (đỏ) = có thể huấn luyện — quy ước màu giữ nguyên như hình gốc, chỉ đổi icon thành chữ tắt để tương thích font khi biên dịch.
...
| 2 (FIX) | BatchNorm của bản sao detection giữ ở eval mode trong suốt huấn luyện | Đúng như script huấn luyện của bài. Đo trực tiếp: BatchNorm ở train mode làm loss kém nhạy 2,5–7 lần với độ sáng/tương phản/màu. **Nhưng sửa điều này không cứu được checkpoint** (R0b ≈ R0) — xem đính chính |
...
- **Nhánh detection: bản sao có thể huấn luyện, nhưng BatchNorm đóng băng ở eval mode.** Đây là kế thừa đúng công thức đã tạo ra checkpoint của bài gốc (script huấn luyện gốc gọi `freeze_bn()` sau mỗi `system.train()`). Đo trực tiếp trên CPU: nếu để BatchNorm chạy ở train mode (thống kê của một batch 2 clip), loss kém nhạy 2,5–7 lần với thay đổi độ sáng/tương phản/màu, và một ảnh giống hệt vẫn bị tính loss khác 0. Đó là một sai khác thật, nhưng **không phải nguyên nhân làm hỏng checkpoint**: R0b (đã sửa) vẫn kém checkpoint gốc +70 điểm BD-rate, gần như bằng R0 (chưa sửa) +75.

  **Đính chính (2026-09-21).** Hai giả thuyết đã nêu trước đây đều bị đo trực tiếp bác bỏ: (a) "BatchNorm ở train mode gây sụp" — R0b ≈ R0; (b) "clone tự trôi khỏi teacher" — chạy detection đóng băng hoàn toàn, không có clone (R3, R4) cho kết quả y hệt bản có clone (R3 ≈ R0b, R4 ≈ R2b, cả Class C và D). Cái dự đoán được độ hỏng là *trọng số của loss detection* (100 % → +70…76 điểm, 50 % → +57…60, 0 % → +23…35). Khác biệt còn lại giữa công thức huấn luyện của bài và của các run này mà đã xác định được là **cách tính rate**: bài tính trên ký hiệu nguyên đã làm tròn, `DMC.forward_train` tính trên giá trị liên tục chưa làm tròn (`multitask_exp/exact_rate.py`). Giả thuyết này **chưa được kiểm chứng bằng bitstream thật** cho đến khi có kết quả R3x/R4x.
- **Nhánh segmentation: đóng băng hoàn toàn, không có bản sao huấn luyện, giám sát tại tầng 17.** Theo đúng bằng chứng CKA (§2.4, §3.1): tại tầng 17 hai tác vụ đã phân kỳ đủ để không thể dùng chung một bản sao nông như detection; đóng băng hoàn toàn là lựa chọn đơn giản nhất (không có tham số nào để điều chỉnh), đổi lại giới hạn khả năng "điều chỉnh" biểu diễn theo yêu cầu riêng của segmentation — đây là một đánh đổi (trade-off), độ lớn của nó là một phần của RQ2/RQ3. Lưu ý: không còn bằng chứng nào cho thấy đóng băng "bền hơn" có clone; hai cách cho cùng kết quả detection.
...
| 5 | Nhánh segmentation đóng băng hoàn toàn, không sao chép | Đơn giản nhất: không có tham số để điều chỉnh. Không dựa vào giả thuyết "clone tự trôi" (đã bị bác bỏ) | Đã đo: đóng băng hay clone cho cùng chất lượng detection (R3 ≈ R0b, R4 ≈ R2b) |
